...
// var gradient = ctx.createLinearGradient(10,0, 200,0);
...
function draw() {
    ...
    for (var i = 0; i < drops.length; i++)
    {
        ...
        ctx.fillText(text, ((i)* font_size+x_offset) % c.width, (drops[i]) * font_size);
        // Incrementing Y coordinate
        drops[i]++;
        // if (i > 0)
        //   drops[i] = drops[i-1]+1;
        // else {
        //   drops[i] = drops[drops.length-1]+1;
        // }
        // sending the drop back to the top randomly after it has crossed the screen
        // adding randomness to the reset to make the drops scattered on the Y axis
       if (drops[i] * font_size > c.height && Math.random() > 0.99) // * font_size > c.height  0.975
			      drops[i] = 1;
    }
    // var tmp = drops[0];
